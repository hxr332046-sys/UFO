...
OUT = Path("G:/UFO/政务平台/data/probe_initdata_busiid.json")
...
def run():
    ...
    s1 = ev(
        r"""(function(){
          ...
          if(!fc) return {err:'no_fc'};
          window.__init_probe={reqs:[],resps:[]};
          ...
          XMLHttpRequest.prototype.send=function(b){
            ...
            if(u.indexOf('/icpsp-api/')>=0){
              window.__init_probe.reqs.push({m:this.__m||'GET',u:u.slice(0,180),len:(b||'').length,body:(b||'').slice(0,220)});
              var self=this;
              self.addEventListener('load',function(){
                window.__init_probe.resps.push({u:u.slice(0,180),status:self.status,text:(self.responseText||'').slice(0,260)});
              });
            }
            return os.apply(this,arguments);
          };
          return {ok:true,hash:location.hash,flowData:(fc.params&&fc.params.flowData)||null};
        })()"""
    )
    ...
    s3 = ev(
        r"""(function(){
          ...
          return {
            ...
            probe:window.__init_probe||null
          };
        })()"""
    )
    rec["steps"].append({"step": "S3_after_inits", "data": s3})
