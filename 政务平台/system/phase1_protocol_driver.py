...
def run(case_path: Path, *, verbose: bool = False) -> int:
    ...
    rec["final"] = {
        "busi_id": c.busi_id,
        "full_name": c.full_name,
        "hit_count": (c.name_check_dto or {}).get("checkResult") and len((c.name_check_dto or {}).get("checkResult") or []) or 0,
    ...
    if exit_code == 0 and not c.busi_id:
        # 全部步骤都 200 但第二次保存没回 busiId — 视作未达里程碑
        exit_code = 4
        rec["error"] = "no_busiid_after_second_save"
        OUT_JSON.write_text(json.dumps(rec, ensure_ascii=False, indent=2), encoding="utf-8")
