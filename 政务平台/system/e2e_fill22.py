...
# 精确路径获取组件
def get_fc():
    return ev("""(function(){var vm=document.getElementById('app').__vue__;return vm.$children[0].$children[0].$children[1].$children[0]})()""")

# ============================================================
# Step 1: initData + 填写基础字段
# ============================================================
print("Step 1: initData + 填写")
ev("""(function(){
...
# ============================================================
# Step 2: 行业类型 - 通过businese-info精确路径
# ============================================================
print("\nStep 2: 行业类型")

# businese-info = fc/3/0/0/0/1
bi = ev("""(function(){
    var fc=document.getElementById('app').__vue__.$children[0].$children[0].$children[1].$children[0];
    var bi=fc.$children[3].$children[0].$children[0].$children[1];
    if(!bi)return{error:'no_bi'};
    var form=bi.busineseForm||{};
    var name=bi.$options?.name||'';
    return{name:name,itemIndustryTypeCode:form.itemIndustryTypeCode||'',industryTypeName:form.industryTypeName||'',busiAreaDataLen:(form.busiAreaData||[]).length};
})()""")
print(f"  businese-info: {bi}")

# 设置行业类型到businese-info form
ev("""(function(){
    var fc=document.getElementById('app').__vue__.$children[0].$children[0].$children[1].$children[0];
    var bi=fc.$children[3].$children[0].$children[0].$children[1];
    var form=bi.busineseForm;
...
# ============================================================
# Step 3: 经营范围 - 通过tni-business-range精确路径
# ============================================================
print("\nStep 3: 经营范围")

# tni-business-range = fc/3/0/0/0/1/0/0/2/0/1/1
tbr = ev("""(function(){
    var fc=document.getElementById('app').__vue__.$children[0].$children[0].$children[1].$children[0];
    var bi=fc.$children[3].$children[0].$children[0].$children[1];
    var rs=bi.$children[0].$children[0].$children[2].$children[0].$children[1];
    var tbr=rs.$children[1];
    if(!tbr)return{error:'no_tbr'};
    var name=tbr.$options?.name||'';
    var al=(tbr.allBusinessList||tbr.$data?.allBusinessList||[]).length;
    var sl=(tbr.searchList||tbr.$data?.searchList||[]).length;
    var il=(tbr.industryList||tbr.$data?.industryList||[]).length;
    var refs=Object.keys(tbr.$refs||{});
    var methods=Object.keys(tbr.$options?.methods||{});
    return{name:name,allBusinessListLen:al,searchListLen:sl,industryListLen:il,refs:refs,methods:methods};
})()""")
...
if isinstance(search_items, list) and len(search_items) > 0:
    ...
    # 通过精确路径调用businese-info.confirm
    confirm_js = '(function(){var fc=document.getElementById("app").__vue__.$children[0].$children[0].$children[1].$children[0];var bi=fc.$children[3].$children[0].$children[0].$children[1];bi.confirm(' + confirm_json + ');})()'
    ev(confirm_js)
...
# ============================================================
# Step 4: 住所cascader - 通过residence-information精确路径
# ============================================================
print("\nStep 4: 住所cascader")
ev("""(function(){
    var fc=document.getElementById('app').__vue__.$children[0].$children[0].$children[1].$children[0];
    var ri=fc.$children[3].$children[0].$children[0].$children[3].$children[0];
    // 设置住所数据
