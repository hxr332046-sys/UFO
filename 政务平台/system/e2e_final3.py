#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""E2E Final3: 强制回首页 → UI点击流程注册动态路由 → 表单 → 填写 → 认证"""
...
print(f"  hash={page.get('hash')} text={page.get('text','')[:60]}")

# 如果还是404，尝试location.href
if '404' in page.get('text','') or '不存在' in page.get('text',''):
    print("  hash导航失败，尝试location.href...")
    ev("location.href=location.origin+location.pathname+'#/index/page'")
    time.sleep(5)
    page = ev("({hash:location.hash, text:(document.body.innerText||'').substring(0,80)})")
    print(f"  hash={page.get('hash')} text={page.get('text','')[:60]}")
...
print(f"  token={verify.get('token')} userInfo={verify.get('userInfo')}")

# ===== STEP 2: 通过首页UI点击进入企业开办专区 =====
print("\nSTEP 2: 通过首页UI进入企业开办专区")
# 找"办理营业执照"或"企业开办"卡片
click1 = ev("""(function(){
    ...
    for(var i=0;i<all.length;i++){
        ...
        if((t.includes('办理营业执照')||t.includes('企业开办'))&&all[i].offsetParent!==null&&all[i].children.length<5){
            all[i].click();
            return{clicked:t.substring(0,15)};
        }
    }
    return{error:'not_found',text:(document.body.innerText||'').substring(0,100)};
})()""")
print(f"  click: {click1}")
time.sleep(5)

page2 = ev("({hash:location.hash, text:(document.body.innerText||'').substring(0,150)})")
print(f"  hash={page2.get('hash')} text={page2.get('text','')[:80]}")

# 如果没到企业专区，尝试Vue Router
if 'enterprise-zone' not in page2.get('hash',''):
    print("  尝试Vue Router导航...")
    ev("""(function(){
        var vm=document.getElementById('app')?.__vue__;
        if(vm?.$router){
    ...
    time.sleep(5)
    page2b = ev("({hash:location.hash, text:(document.body.innerText||'').substring(0,100)})")
    print(f"  hash={page2b.get('hash')} text={page2b.get('text','')[:60]}")
...
page3 = ev("({hash:location.hash, formCount:document.querySelectorAll('.el-form-item').length, text:(document.body.innerText||'').substring(0,100)})")
print(f"  hash={page3.get('hash')} forms={page3.get('formCount',0)} text={page3.get('text','')[:60]}")

# ===== STEP 4: 在without-name/select-prise页面调用getHandleBusiness =====
print("\nSTEP 4: 调用getHandleBusiness注册namenotice路由")
result = ev("""(function(){
    ...
    if(!inst)return{error:'no_getHandleBusiness',routeName:routeName,matchedNames:matched.map(function(m){return m.name||''})};
    
    // 调用getHandleBusiness
    inst.getHandleBusiness({entType:'1100'});
    return{called:true,routeName:routeName};
})()""")
...
print(f"  hash={page4.get('hash')} forms={page4.get('formCount',0)} inputs={page4.get('inputCount',0)}")

# 等待
fc = page4.get('formCount',0)
if fc == 0:
...
if fc == 0:
    # 如果getHandleBusiness导航到了namenotice但表单还没加载
    # 尝试直接导航到flow路由
    print("  尝试直接导航到flow路由...")
    nav2 = ev("""(function(){
        var vm=document.getElementById('app')?.__vue__;
        ...
        return{resolved:m.route.path,name:m.route.name||'',matched:m.route.matched?.length||0};
    })()""")
    print(f"  flow route resolve: {nav2}")
    
    if nav2 and nav2.get('resolved') != '/404':
        ev("""(function(){
            var vm=document.getElementById('app')?.__vue__;
            try{vm.$router.push('/flow/base/basic-info')}catch(e){}
        })()""")
        time.sleep(5)
        fc = ev("document.querySelectorAll('.el-form-item').length") or 0
        print(f"  after flow nav: forms={fc}")
...
if fc == 0:
    ...
    debug = ev("""(function(){
        ...
        for(var i=0;i<matched.length;i++){
            ...
            info.push({path:m.path,name:m.name||'',hasInst:!!inst,methods:inst?Object.keys(inst.$options?.methods||{}).slice(0,10):[]});
        }
        return{route:route?.path,matched:info,hash:location.hash,text:(document.body.innerText||'').substring(0,150)};
    })()""")
    print(f"  debug: {json.dumps(debug or {}, ensure_ascii=False)[:500]}")
    log("85.表单加载失败", debug or {})
    ws.close()
    sys.exit(1)
...
if form:
    log("85.设立登记表单", {"formCount":len(form.get("fields",[])),"steps":form.get("steps",[]),"buttons":form.get("buttons",[]),"stepTitle":form.get("stepTitle","")})
    log("85a.字段详情", {"fields":form.get("fields",[])[:60]})
...
log("86.填写测试",{"ok":len(ok),"fail":len(fail),"ok_list":ok,"fail_list":fail},
...
for step in range(8):
    ...
    if current.get('hasSubmit'):
        print(f"\n  🔴 检测到提交按钮！停止（不点击提交）")
        log("87.提交按钮检测", {"step":step,"stepTitle":step_info.get('title',''),"auth":auth,"buttons":current.get('buttons',[])})
    ...
    if current.get('hasNext'):
        ...
        if errs:
            print(f"  ⚠️ 验证错误: {errs}")
            log(f"87b.step{step}_errors", {"errors":errs})
            break
    else:
        print(f"  无下一步按钮，停止")
        break
